...
def SubMatrizes(A,B, m, n):
    ...
    for i in range (m):                         # Loop interno para as linhas
        for j in range(n):                      # Loop externo para as colunas
            C[i][j] = A[i][j] - B[i][j]         # Subtrai elemento a elemento
    
    return C                                    # Retorna a matriz resultante da subtração de A e B      
...
try:                                            # Try sendo usado para controlar se A e B possuem a mesma dimensão
    ...
    C = SubMatrizes(A,B,m1,n1)                                                     # Chama a função para subtrair as duas matrizes, caso possuam mesma dimensão
    print("A subtração das matrizes é C:")                                               # Mostra o resultado
    print(C)
